...
**模式一：護欄「有實作、無執行點」。** `x402-agent-payments` 的 `verifier/x402_v2/rate_limiter.py` 有完整的滑動視窗限流實作、有 5 個單元測試，但**全 repo 零呼叫端**——它沒有接上任何請求路徑。而 `verifier/README.md` 的測試表把它列為「provider-side rate limiting」。也就是說：看文件你會以為有速率限制，看程式碼你會發現它是一座孤島。這種「有規則、無執行點」在盤點時最容易漏，因為它「有寫」。
...
- **我沒有修任何一個被盤點的 repo。** 落差先記錄、先講清楚，修不修是另一個決定——把「發現」和「修補」分開，是我在文件治理上的一貫做法。
